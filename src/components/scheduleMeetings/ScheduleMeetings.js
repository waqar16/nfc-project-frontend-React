...
const ScheduleMeeting = ({ attendeeEmail, userId }) => {
  ...
  const handleAuthAndSchedule = async () => {
    ...
    try {
      ...
      const authUrl = `https://letsconnect.onesec.shop/google/auth-request/?title=${encodeURIComponent(meetingDetails.title)}&description=${encodeURIComponent(meetingDetails.description)}&start_datetime=${combinedDateTime}&attendee_email=${encodeURIComponent(attendeeEmail)}&user_id=${encodeURIComponent(userId)}`;
