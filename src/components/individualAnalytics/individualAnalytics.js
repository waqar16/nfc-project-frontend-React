...
const Analytics = () => {
  ...
  const fetchInteractionFrequency = async (frequency) => {
    try {
      setLoading(true);
      const response = await axios.get(`https://api.onese.shop/api/interaction-frequency/${frequency.toLowerCase()}`, {
        headers: {
  ...
  const fetchPeakInteractionTime = async (frequency) => {
    try {
      ...
      const response = await axios.get(`  https://api.onese.shop/api/peak-interaction-time/${formattedFrequency}`, {        headers: {
          Authorization: `Token ${localStorage.getItem('authToken')}`,
        },
      });
