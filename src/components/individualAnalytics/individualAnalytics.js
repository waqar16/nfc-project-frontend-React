...
const Analytics = () => {
  ...
  const fetchInteractionFrequency = async (frequency) => {
    try {
      setLoading(true);
      const response = await axios.get(`https://54.84.254.221/api/interaction-frequency/${frequency.toLowerCase()}`, {
        headers: {
  ...
  const fetchPeakInteractionTime = async (frequency) => {
    try {
      ...
      const response = await axios.get(`  https://54.84.254.221/api/peak-interaction-time/${formattedFrequency}`, {        headers: {
          Authorization: `Token ${localStorage.getItem('authToken')}`,
        },
      });
