...
const DigitalProfile = () => {
  ...
  const fetchReceivedCards = useCallback(async () => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await axios.get('http://127.0.0.1:8000api/received-cards/', {
        headers: {
          Authorization: `Token ${token}`
        }
      });
      const cards = await Promise.all(response.data.map(async (card) => {
        const userResponse = await axios.get(`http://127.0.0.1:8000auth/users/${card.shared_from}/`, {
          headers: {
            Authorization: `Token ${token}`
          }
        });
  ...
  const handleShareToCard = async () => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await axios.post('http://127.0.0.1:8000api/share-profile-url/', {}, {
        headers: {
          Authorization: `Token ${token}`
        }
      });
  ...
  const handleWriteToNFC = async () => {
    try {
      const token = localStorage.getItem('authToken');
      axios.post('http://127.0.0.1:8000api/nfc-write/', user, {
        headers: {
          Authorization: `Token ${token}`,
        },
      });
  ...
  const handleShareProfile = async (recipient) => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await axios.post('http://127.0.0.1:8000api/share-profile/', { shared_to: recipient }, {
        headers: {
          Authorization: `Token ${token}`,
        },
      });
