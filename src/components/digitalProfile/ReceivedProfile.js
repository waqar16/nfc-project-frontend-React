...
const ReceivedProfile = () => {
  ...
  const handleGoogleSuccess = async (response) => {
    ...
    try {
      ...
      const res = await axios.post('https://api.onese.shop/auth/custom-google-login/', {
  ...
  const fetchUserData = useCallback(async () => {
    try {
      ...
      const endpoint = profileType === 'employee' ? `  https://api.onese.shop/api/employees/${email}/` : `  https://api.onese.shop/api/profiles/${userId}/`;
  ...
  const createInteraction = async (user_id) => {
    try {
      ...
      await axios.post(
        '  https://api.onese.shop/api/create_interaction/',
        {
          user: user_id,
  ...
  const shareProfile = async () => {
    ...
    try {
      ...
      try {
        const endpoint = profile_type === 'employee' ? `https://api.onese.shop/api/employees/${email}/` : `  https://api.onese.shop/api/profiles/${userId}/`;
        ...
        if (error.response && error.response.status === 404) {
          await axios.post('https://api.onese.shop/api/profiles/', {
            user: id,
      ...
      await axios.post('https://api.onese.shop/api/share-profile/', { shared_to: recipient }, {
        headers: {
          Authorization: `Token ${token}`,
        },
      });
