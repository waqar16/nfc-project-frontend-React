...
const TeamManagement = () => {
  ...
  const fetchCompanyAndEmployees = async () => {
    try {
      ...
      const companyResponse = await axios.get('  https://api.onese.shop/api/companies/', {
        headers: {
  ...
  const fetchEmployees = async (token) => {
    try {
      const employeeResponse = await axios.get('https://api.onese.shop/api/employees/', {
        headers: {
          Authorization: `Token ${token}`,
        },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`https://api.onese.shop/api/employees/${employee.email}/`, employee, {
          headers: {
            Authorization: `Token ${token}`,
          },
        });
        ...
        await axios.post('https://api.onese.shop/api/employees/', employee, {
          headers: {
            Authorization: `Token ${token}`,
          },
        });
  ...
  const handleDelete = async (email) => {
    const token = localStorage.getItem('authToken');
    try {
      await axios.delete(`https://api.onese.shop/api/employees/delete/${email}/`, {
        headers: {
          Authorization: `Token ${token}`,
        },
      });
