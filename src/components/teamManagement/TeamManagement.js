...
const TeamManagement = () => {
  ...
  const fetchCompanyAndEmployees = async () => {
    try {
      ...
      const companyResponse = await axios.get('  https://54.84.254.221/api/companies/', {
        headers: {
  ...
  const fetchEmployees = async (token) => {
    try {
      const employeeResponse = await axios.get('https://54.84.254.221/api/employees/', {
        headers: {
          Authorization: `Token ${token}`,
        },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`https://54.84.254.221/api/employees/${employee.email}/`, employee, {
          headers: {
            Authorization: `Token ${token}`,
          },
        });
        ...
        await axios.post('https://54.84.254.221/api/employees/', employee, {
          headers: {
            Authorization: `Token ${token}`,
          },
        });
  ...
  const handleDelete = async (email) => {
    const token = localStorage.getItem('authToken');
    try {
      await axios.delete(`https://54.84.254.221/api/employees/delete/${email}/`, {
        headers: {
          Authorization: `Token ${token}`,
        },
      });
