  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        try {
          const profileResponse = await axios.get(`http://127.0.0.1:8000api/employees/${email}/`, {
            headers: {
              Authorization: `Token ${token}`,
            },
          });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (profileExists) {
        ...
        await axios.put(`http://127.0.0.1:8000api/employees/${user.email}/`, formData, {
          headers: {
        ...
        await axios.post('http://127.0.0.1:8000api/employees/', formData, {
          headers: {
            Authorization: `Token ${authToken}`,
            'Content-Type': 'multipart/form-data',
          },
      ...
      const profileResponse = await axios.get(`http://127.0.0.1:8000api/employees/${user.email}/`, {
        headers: {
          Authorization: `Token ${authToken}`,
        },
      });
