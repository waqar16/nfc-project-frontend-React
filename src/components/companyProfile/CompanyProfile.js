...
const CompanyProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchCompanyData = async () => {
      try {
        ...
        try {
          const companyResponse = await axios.get(`http://127.0.0.1:8000api/companies/${userId}/`, {
            headers: {
              Authorization: `Token ${token}`,
            },
          });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (profileExists) {
        await axios.put(`http://127.0.0.1:8000api/companies/${userId}/`, company, {
          headers: {
        ...
        const createResponse = await axios.post('http://127.0.0.1:8000api/companies/', company, {
          headers: {
            Authorization: `Token ${authToken}`,
          },
        });
